...
class FasterWhisperPipeline(Pipeline):
    ...
    def detect_segment_language_whisper(self, segment_audio: np.ndarray) -> str:
        """
        Whisper 모델을 이용하여 특정 segment(음성 구간)의 언어를 감지
        """
        if len(segment_audio) < SAMPLE_RATE * 1:  # 최소 1초 이상의 오디오가 필요
            return "unknown"

        # Whisper 모델의 멜 스펙트로그램 크기 확인
        model_n_mels = self.model.feat_kwargs.get("feature_size", 80)  # 기본값 80

        # Whisper 모델을 이용해 segment의 언어 감지
        mel_spectrogram = log_mel_spectrogram(segment_audio, n_mels=model_n_mels)
        encoder_output = self.model.encode(mel_spectrogram)
        results = self.model.model.detect_language(encoder_output)

        # 가장 높은 확률을 가진 언어 선택
        language_token, language_probability = results[0][0]
        detected_language = language_token[2:-2]  # "<|en|>" 같은 형식을 "en"으로 변환

        print(f"Segment detected language: {detected_language} ({language_probability:.2f})")

        return detected_language if language_probability > 0.5 else "ko"
    ...
    def transcribe(
        ...
        for idx, out in enumerate(self.__call__(data(audio, vad_segments), batch_size=batch_size, num_workers=num_workers)):
            ...
            # 해당 segment의 오디오 추출
            f1 = int(vad_segments[idx]['start'] * SAMPLE_RATE)
            f2 = int(vad_segments[idx]['end'] * SAMPLE_RATE)
            segment_audio = audio[f1:f2]

            # Whisper 모델을 이용해 segment별 언어 감지
            detected_lang = self.detect_segment_language_whisper(segment_audio)
...
def load_model(
    ...
    if os.path.exists(whisper_arch):
        print(f"Local model loading: {whisper_arch}")
        ...
        print(f"🔍 Hugging Face model loading: {whisper_arch}")
        model = model or WhisperModel(
            whisper_arch,  
            device=device,
            device_index=device_index,
